...
struct MANGOS_DLL_DECL mob_dark_rider_of_acherusAI : public ScriptedAI
{
    ...
    {
        if (Unit* pPlayer = m_creature->GetMap()->GetPlayer(uiPlayerGUID))
...
struct MANGOS_DLL_DECL npc_highlord_darion_mograineAI : public npc_escortAI
{
    ...
    void Reset()
    {
        if (!HasEscortState(STATE_ESCORT_ESCORTING))
        {
            ...
            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiTirionGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiKorfaxGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiMaxwellGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiEligorGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiRayneGUID)))
                pTemp->setDeathState(JUST_DIED);
            ...
            {
                if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiDefenderGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiDefenderGUID[i] = 0;
            }
            for(uint8 i = 0; i < ENCOUNTER_EARTHSHATTER_NUMBER; ++i)
            {
                if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiEarthshatterGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiEarthshatterGUID[i] = 0;
            }

            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiKoltiraGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiOrbazGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiThassarianGUID)))
                pTemp->setDeathState(JUST_DIED);
            if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiLichKingGUID)))
                pTemp->setDeathState(JUST_DIED);
            ...
            {
                if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiAbominationGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiAbominationGUID[i] = 0;
            }
            for(uint8 i = 0; i < ENCOUNTER_BEHEMOTH_NUMBER; ++i)
            {
                if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiBehemothGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiBehemothGUID[i] = 0;
            }
            for(uint8 i = 0; i < ENCOUNTER_GHOUL_NUMBER; ++i)
            {
                if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiGhoulGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiGhoulGUID[i] = 0;
            }
            for(uint8 i = 0; i < ENCOUNTER_WARRIOR_NUMBER; ++i)
            {
                if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiWarriorGUID[i])))
                    pTemp->setDeathState(JUST_DIED);
                uiWarriorGUID[i] = 0;
            }
        }
    ...
    void WaypointReached(uint32 i)
    {
        switch(i)
        {
            ...
            case 3:
            {
                Creature* pTirion = ((Creature*)m_creature->GetMap()->GetCreature(uiTirionGUID));
            ...
            if (uiPhase_timer < diff)
            {
                ...
                switch(uiStep)
                {
                    ...
                    case 3: // rise
                    {
                        ...
                        // and lets player chose if he/she continues the quest or if he ignores the event.
                    ...
                    case 16: // Alexandros out
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiAlexandrosGUID)))
                        {
                    ...
                    case 19: // runs to father
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiDarionGUID)))
                        {
                    ...
                    case 24:
                        ...
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiTirionGUID))) // Tirion moves forward here
                    ...
                    case 29: // lich king spawns
                        if (Creature* pTemp = m_creature->SummonCreature(NPC_THE_LICH_KING, LightofDawnLoc[26].x, LightofDawnLoc[26].y, LightofDawnLoc[26].z, LightofDawnLoc[26].o, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
                        {
                            ...
                            if (Creature* pAlex = ((Creature*)m_creature->GetMap()->GetCreature(uiAlexandrosGUID)))
                    ...
                    case 30:
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiAlexandrosGUID))) // remove him
                        {
...
// disable              if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiLichKingGUID)))
...
//   doesn't make it looks well, so workarounds, Darion charges, looks better
                    ...
                    case 41: // Lich king - Apocalypse
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiLichKingGUID)))
                        {
                            ...
                            if (Creature* pTirion = ((Creature*)m_creature->GetMap()->GetCreature(uiTirionGUID)))
                            {
                    ...
                    case 43: // They all got kicked
                        ...
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiDefenderGUID[0])))
                        {
                            pTemp->SetSpeedRate(MOVE_RUN, 6.0f);
                            ((Unit*)pTemp)->SetStandState(UNIT_STAND_STATE_DEAD);
                            pTemp->GetMotionMaster()->MovePoint(0, LightofDawnLoc[0].x+rand()%10, LightofDawnLoc[0].y+rand()%10, LightofDawnLoc[0].z);
                        }
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiEarthshatterGUID[0])))
                        {
                            pTemp->SetSpeedRate(MOVE_RUN, 6.0f);
                            ((Unit*)pTemp)->SetStandState(UNIT_STAND_STATE_DEAD);
                            pTemp->GetMotionMaster()->MovePoint(0, LightofDawnLoc[0].x+rand()%10, LightofDawnLoc[0].y+rand()%10, LightofDawnLoc[0].z);
                    ...
                    case 59:
                        ...
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiTirionGUID))) // Tirion runs to Darion
                        {
                            pTemp->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_ONESHOT_NONE);
                            pTemp->SetSpeedRate(MOVE_RUN, 1.0f);
                    ...
                    case 60:
                        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiLichKingGUID))) // Lich king disappears here
                        {
    ...
    {
        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(ui_GUID)))
            if (pTemp->isAlive())
    ...
    void SpawnNPC()
    {
        ...
        for(uint8 i = 0; i < ENCOUNTER_GHOUL_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiGhoulGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_ABOMINATION_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiAbominationGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_WARRIOR_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiWarriorGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_BEHEMOTH_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiBehemothGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_DEFENDER_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiDefenderGUID[i]))))
            {
        ...
        for(uint8 i = 0; i < ENCOUNTER_EARTHSHATTER_NUMBER; ++i)
        {
            if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiEarthshatterGUID[i]))))
            {
        ...
        if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiKorfaxGUID))))
        {
        ...
        if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiMaxwellGUID))))
        {
        ...
        if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiEligorGUID))))
        {
        ...
        if (!(pTemp = ((Creature*)m_creature->GetMap()->GetCreature(uiRayneGUID))))
        {
    ...
    {
        if (Creature* pTemp = ((Creature*)m_creature->GetMap()->GetCreature(pGUID)))
            if (pTemp->isAlive())
            {
